...
# Due Date: 3.17.2023
# Description: HashMap implementation using quadratic probing with the following methods: put(), empty_buckets(), table_load(),
        ...
        while i < self._capacity and hash_entry is not None:
            if hash_entry.key == key:
                ...
                while self._buckets[j] is not None:
                    ...
                    self._size -= 1
                    reinsert_index = self._hash_function(next_entry.key) % self._capacity
                    k = 0
                    while k < self._capacity:
                        reinsert_index = (self._hash_function(next_entry.key) + k * k) % self._capacity
                        if self._buckets[reinsert_index] is None:
                            self._buckets[reinsert_index] = next_entry
                            self._size += 1
                            break
                        k += 1
